...
class NWTDirection(ExtendedDDS, GurobiSettings):
    ...
    def compute_direction(self, problem: ExtendedProblem, Jac: np.array, x_p: np.array = None, Hes: np.array = None):
        assert x_p is None and Hes is not None

        m, n = Jac.shape

        reset = False

        if np.isinf(Jac).any() or np.isnan(Jac).any() or np.isinf(Hes).any() or np.isnan(Hes).any():
            return np.zeros(n), 0, reset

        for j in range(m):
            eigvals = np.linalg.eigvals(Hes[j, :, :])
        ...
        if reset:
            if self._verbose:
                print('Reset')
            Hes = np.empty((m, n, n), dtype=np.float)
            for j in range(m):
                Hes[j, :, :] = np.eye(n, dtype=np.float)

        try:
            model = Model("Newton Direction")
            ...
            for j in range(m):
                linear_expression = Jac[j, :] @ d + 1/2 * (d @ Hes[j, :, :] @ d) - t
                model.addConstr(linear_expression <= 0, name='Jacobian/Hessian constraint n°{}'.format(j))
