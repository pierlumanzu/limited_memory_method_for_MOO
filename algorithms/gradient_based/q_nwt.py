...
class QNWT(ExtendedGradientBasedAlgorithm):
    ...
    def __init__(self,
        ...
        self._direction_solver = DirectionSolverFactory.get_direction_calculator('QNWTDirection', verbose, gurobi_method, gurobi_verbose)
    ...
    def search(self, p_list: np.array, f_list: np.array, problem: ExtendedProblem):
        ...
        for i in range(n_points):
            ...
            self.__B = np.empty((m, n, n), dtype=np.float)
            for j in range(m):
                self.__B[j, :, :] = np.eye(n, dtype=np.float)
            ...
            while not self.evaluate_stopping_conditions() and self._theta < self._theta_tol and self._alpha != 0 and enter_while:
                ...
                v, self._theta, reset = self._direction_solver.compute_direction(problem, J, None, self.__B)
                ...
                if self._theta < self._theta_tol:
                    ...
                    while not exit_line:
                        ...
                        if self._alpha != 0:
                            ...
                            self.approx_hes(J, p_list[i, :], new_J, new_p)
                            J = new_J
    ...
    def approx_hes(self, J_ini: np.array, p_ini: np.array, J_fin: np.array, p_fin: np.array):
        m, n = J_ini.shape
        ...
        for j in range(m):
            ...
            if sy > 0:
                ...
                self.__B[j, :, :] = self.__B[j, :, :] - \
                                    (np.dot(self.__B[j, :, :], np.dot(s[:, None], np.dot(s[None, :], self.__B[j, :, :])))) / (np.dot(s, np.dot(self.__B[j, :, :], s))) + \
                                    (np.dot(y[j, :][:, None], y[j, :][None, :])) / sy
                ...
                ro = 1 / (np.max(np.dot(J_fin, s)) - np.dot(J_ini[j, :], s))

                ro_sy = ro ** -1 - sy
                sBs = np.dot(s, np.dot(self.__B[j, :, :], s))

                den = ro_sy ** 2 + ro ** -1 * sBs

                ysB = np.dot(y[j, :][:, None], np.dot(s, self.__B[j, :, :])[None, :])

                self.__B[j, :, :] = self.__B[j, :, :] - \
                                    (ro ** -1 * np.dot(self.__B[j, :, :], np.dot(s[:, None], np.dot(s[None, :], self.__B[j, :, :])))) / den + \
                                    (sBs * np.dot(y[j, :][:, None], y[j, :][None, :])) / den + ro_sy * (ysB + ysB.T) / den

    def reset_all(self):
        ExtendedGradientBasedAlgorithm.reset_all(self)
        self.reset_approx_hes()

    def reset_approx_hes(self):
        self.__B = None
